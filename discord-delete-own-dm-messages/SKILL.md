...
- Use Chrome DevTools MCP for Discord Web deletion. It can inspect pages and run bounded page scripts in the existing Chrome profile without OS-level UI automation.
...
Use Discord Web API ownership first. Delete only messages whose API `author.id` matches `/api/v9/users/@me`.

Use visible UI ownership signals only when doing a read-only page check or an explicitly bounded manual fallback:
...
- Use the Fast API Pattern for normal deletion. Do not maintain a parallel UI deletion workflow for bulk cleanup.
- Use UI interaction only as an explicitly bounded fallback for one or two messages when the API path is unavailable and the user has approved that fallback.
- Prefer API scans over hovering visible articles. Use author-group UI scans only for read-only verification or the bounded fallback above.
- For the bounded UI fallback, account for Discord virtual scrolling. A message may be partly visible or very tall, so choose a hover point inside the visible clipped portion of the article rather than relying on the article's geometric center.
...
- For the bounded UI fallback, maintain an in-batch skip set keyed by the visible article id or stable text/time signature for call-history rows, rows that expose no `Edit`, and rows whose ownership is ambiguous. Do not retry those rows until after a scroll or page reload changes the loaded message set.
- For the bounded UI fallback, pre-scan visible rows before hovering. Only hover rows that pass all cheap checks: same DM, non-call-history row, carried user author group, and not already in the skip set.
- Cap each destructive browser call by work, not only deletions: use API residual-rescan batches of up to 50 candidates or around 45 seconds. For the bounded UI fallback, use one or two messages per batch. Return counts before starting the next batch.
- For the bounded UI fallback, close any open context menu after a skipped row or failed deletion attempt, then wait briefly for the message list to settle before continuing. Open menus left behind can make Discord's virtualized list slower and can steal later clicks.
...
- Use `evaluate_script` for dry-run scans, deletion batches, and residual scans. Do not return message bodies.
- Verify `location.href`, the target channel ID, `/api/v9/users/@me`, and `/api/v9/channels/<channel-id>` before deleting.
- Use the API message list as the source of truth for ownership and message type.
- Keep the newest own message ID in an explicit keep set when the request says to leave the latest message.
- Delete in residual-rescan batches. Each batch should rescan from the newest message, collect the next undeleted candidate IDs, delete up to 50 candidates or until the wall-clock budget is reached, and return counted results.
- Respect `429 retry_after` exactly and continue only after the server-approved wait.
- After all requested deletion batches, run a full residual scan and require residual deletable count to be zero before reporting full cleanup.

## Fast API Pattern

Use a Chrome DevTools MCP `evaluate_script` that calls Discord Web API from inside the already logged-in page. This avoids OS automation and avoids returning credentials.
...
- Prefer residual-rescan batches for reliability. Each batch should rescan from the newest message, collect the next undeleted candidate IDs, delete up to 50 candidates or a bounded time budget, then return counted results. This avoids cursor loss when a timed batch stops early.
...
3. Run the Fast API Pattern dry-run.
4. If the dry-run finds unknown owned message types, stop and report them instead of deleting.
5. Run live deletion in bounded residual-rescan batches of up to 50 candidates or the wall-clock budget. Respect `429 retry_after` exactly.
6. Verify with a full API residual scan after live-run.
7. Use UI interaction only as an explicitly approved fallback for one or two messages when the API path is unavailable. Stop immediately if the URL changes, the DM header changes, ownership cannot be verified, Discord shows an unexpected modal, or browser-control calls time out before returning a counted result.

## Verification

After deleting the requested count, run a strict final API residual scan:

1. Verify the current URL and channel metadata still match the target DM.
2. Scan all message pages from newest to oldest through `/api/v9/channels/<channel-id>/messages?limit=100`.
3. Treat a message as residual user-owned content only when `author.id` matches the current user and the type is allowed by the Message Type Policy.
...
5. If any residual user-owned message is found and the user has authorized full cleanup, delete it using the Fast API Pattern, then restart the strict residual scan.
6. Finish only after a strict residual scan reaches the end with zero residual deletable messages.
